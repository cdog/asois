...
@section('tabs')
  <ul class="nav nav-tabs">
    <li class="active" role="presentation"><a href="{{ URL::route('poll', array($poll->id)) }}">Poll</a></li>
    ...
    <div class="col-sm-6 col-sm-offset-3 col-lg-4 col-lg-offset-4">
      ...
      <div class="panel panel-default">
        <div class="panel-body">
          @if ( ! $poll->open || $poll->answered)
            <h4>{{{ $poll->question }}}</h4>
            <hr>
            @foreach ($answers as $answer)
              <div class="radio">
                <label>
                  {{ Form::radio('answer', $answer->id, $answer->id == $poll->answered, array('disabled')) }}
                  {{{ $answer->answer }}}
                </label>
              </div>{{-- .radio --}}
            @endforeach
