...
@section('tabs')
  <ul class="nav nav-tabs">
    ...
    @if ($poll->answered || (Auth::check() && Auth::user()->can('read_poll_results')))
      <li class="active" role="presentation"><a href="{{ URL::route('results', array($poll->id)) }}">Results</a></li>
    @endif
    @if (Auth::check() && Auth::user()->can('read_poll_statistics'))
      <li role="presentation"><a href="{{ URL::route('statistics', array($poll->id)) }}">Statistics</a></li>
    @endif
  </ul>{{-- .nav .nav-tabs --}}
@stop
      ...
      <div class="panel panel-default">
        <div class="panel-body">
          ...
          @foreach ($answers as $answer)
            <p>{{{ $answer->answer }}}</p>
            <div class="progress">
              <div class="progress-bar" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: {{ $answer->results }}%;">
                {{ $answer->results }}%
              </div>
            </div>{{-- .progress --}}
          @endforeach
